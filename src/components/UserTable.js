...
//   let actionList = []
  ...
  const verifyColumn = [
    {
      ...
      renderCell: (params) => {
        return (
            <div>
                <ActionLink onClick={() => setVerified(prev => !prev)}>
  ...
  const blockColumn = [
    ...
    //   renderCell: (params) => {
    //     return (
    //         <ActionLink onClick={() => setBlocked(prev => !prev)}>
  ...
  return (
    <>
      <h1 className="text-center my-5 text-muted">Administrators list</h1>
      {userList?.length === 0 
        ...
        <DataTable>
            <DataGrid
            rows={userList}
            columns={userColumns.concat(modifyColumn, verifyColumn, blockColumn)}
            pageSize={20}
